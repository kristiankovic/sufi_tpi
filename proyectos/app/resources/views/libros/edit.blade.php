...
@section('content')
    <div class="bg-gray-400 flex flex-col justify-center items-center mx-8 gap-3 p-4">
        ...
        <form class="grid grid-rows-4 gap-3" action="{{ route('libros.update', $libro->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div>
                <label for="titulo">Título</label>
                <input class="bg-white w-full p-1" type="text" name="titulo" id="titulo" value="{{ $libro->titulo }}"
                    required>
            </div>

            <div>
                <label for="anio_publicacion">Año de publicación</label>
                <input class="bg-white w-full p-1" type="number" name="anio_publicacion" id="anio_publicacion"
